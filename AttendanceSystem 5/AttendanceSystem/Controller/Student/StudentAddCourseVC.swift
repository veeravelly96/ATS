...
class StudentAddCourseVC: UIViewController {
    ...
    func validateData() {
        ...
        else if let subjects = AppStateManager.shared.loggedInUser?.student?.subjects, subjects.contains(where: { ($0.id == self.selctedSubjectId && $0.section == selctedSection)}) {
            shouldProceed = false
    ...
    func aadCourse() {
        let course =  SubjectModel.init(id: selctedSubjectId, name: subjectTF.text!, semester: AppStateManager.shared.loggedInUser?.student?.semester ?? .Fall, section: selctedSection)
        
        var subjects = [SubjectModel]()
        if let sbjcts = AppStateManager.shared.loggedInUser?.student?.subjects {
           subjects = sbjcts
        }
        ...
        do {
            ...
            ref.child("\(dbPath)/\(AppStateManager.shared.getStudentId())/subjects")
                .setValue(json)
            
            Utility.showAlert(title: "Alert", message: "Course added successfully", okTapped: {
                AppStateManager.shared.loggedInUser?.student?.subjects = subjects
